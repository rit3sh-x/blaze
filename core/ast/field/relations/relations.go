...
func (rv *RelationValidator) ParseRelationFromString(definition string, fromClass string, toClass string) (*Relation, error) {
	...
	if matches == nil || strings.TrimSpace(matches[1]) == "" || strings.TrimSpace(matches[2]) == "" {
	...
	if len(sourceFields) == 0 {
		return nil, fmt.Errorf("source field cannot be empty")
	}
...
func (r *Relation) String() string {
	...
	parts = append(parts, fmt.Sprintf("[%s], [%s]", strings.Join(r.From, ", "), strings.Join(r.To, ", ")))
...
func (r *Relation) ValidateOneToOneRelation(isSourceOptional bool) error {
	if r.IsComposite() {
		return fmt.Errorf("one-to-one relations typically don't use composite keys")
	}
...
func (r *Relation) ValidateCompositeRelation() error {
	...
	targetFields := r.GetTargetFields()
...
func (r *Relation) GetRelationKey() string {
	key := fmt.Sprintf("%s->%s", r.From, strings.Join(r.To, ","))
	if r.HasName() {
		key += ":" + r.Name
	}
	return key
